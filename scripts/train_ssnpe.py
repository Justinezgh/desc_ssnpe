...
sample_ff = jnp.load(
  './data/posterior_full_field__256N_10ms_27gpa_0.26se.npy'
)
...
if nb_simulations_allow != 100_000: 
  ...
  sample_ref_sbi = jnp.load(
    './exp{}/posteriors_sample.npy'.format(
     PATH_REF
    )
  )
...
def loss_nll(
    ...
    weight = jnp.clip(1./jnp.exp(weight - prob_max), 0, 500)

    lp, out = jax.vmap(
      jax.value_and_grad(lambda theta, x: log_prob_fn(params, theta.reshape([1,dim]), x.reshape([1,dim])).squeeze())
    )(mu, batch)

    return - jnp.mean(lp * weight) + weight_score * jnp.sum((out - score)**2, axis=-1).mean()
...
for batch in pbar:
  ...
  if not jnp.isnan(ex_y).any():
    ...
    if batch % 5000 == 0:
      # save params
      with open("./exp{}/save_params/params_ode_flow.pkl".format(PATH), "wb") as fp:
      ...
      plt.savefig(
        './exp{}/fig/contour_plot_step{}'.format(
          PATH, batch
        )
      )
...
import csv
field_names = ['experiment_id', 'activ_fun', 'lr_schedule', 'total_steps', 'nb_simulations', 'score_weight','n_flow_layers','n_bijector_layers', 'c2st', 'nlp', 'seed']
dict = {
...
with open('./metric_deep_ensemble.csv', 'a') as csv_file:
